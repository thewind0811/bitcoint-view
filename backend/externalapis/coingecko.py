...
from assets.asset import AssetWithOracles, Asset
...
from constants.resolver import strethaddress_to_identifier, evm_address_to_identifier
from crypto.models import PriceHistory
from errors.asset import UnsupportedAsset, UnknownAsset
from errors.misc import RemoteError
from errors.price import NoPriceForGivenTimestamp, PriceQueryUnsupportedAsset
from fval import FVal
from history.types import HistoricalPriceOracle
from intefaces import HistoricalPriceOracleInterface
from types import Price, Timestamp, ChainID, EvmTokenKind
from utils.misc import set_user_agent, ts_now
...
class Coingecko(HistoricalPriceOracleInterface, PenalizablePriceOracleMixin):
    ...
    def _query(
        ...
        if response.status_code != 200:
            msg = (
                f'Coingecko API reqeust {response.url} failed with HTTP status '
    ...
    def asset_data(self, asset_coingecko_id: str) -> CoingeckoAssetData:
        ...
        - Remote Error if there is a problem querying coingecko
        """
        options = {
            'localization': 'false',
            'tickers': 'fasle',
        ...
        try:
            ...
            raise RemoteError(
                f'Missing expected key entry {e} in coingecko data response'
            ) from e
    ...
    def check_vs_currencies(
        ...
        if vs_currency not in COINGECKO_SIMPLE_VS_CURRENCIES:
            log.warning(
                ...
                f'to {to_asset.identifier}. But to_asset not supported.'
            )
            return None
        return vs_currency
    ...
    def get_current_price(
            ...
            from_asset: AssetWithOracles,
            to_asset: AssetWithOracles,
            match_main_currenty: bool
    ) -> tuple[Price, bool]:
        """
        Returns a simple price for from_asset in coingecko and `False` value
        since it never tries to match main

        Use the simple/price endpoint of coingecko. If to_asset is not part of the
    ...
    def query_historical_price(
        ...
        price_cache_entry = PriceHistory.objects.filter(
            from_asset=from_asset,
            to_asset=to_asset,
            timestamp=timestamp,
