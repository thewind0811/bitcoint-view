...
from rest_framework.exceptions import PermissionDenied, APIException, MethodNotAllowed
...
class AsyncAPIConsumer(AsyncJsonWebsocketConsumer, metaclass=APIConsumerMetaclass):
    ...
    # The following policies any be set at either globally, or per-view
    ...
    async def add_group(self, name: str):
        """
        Add a group to the set of groups this consumer is subscribing to
        """
    ...
    async def handle_action(self, action: str, request_id: str, **kwargs):
        ...
        If here is no action listed on the consumer for this action name
    ...
    def call_view(self, action: str, **kwargs):
        ...
        if isinstance(response, Response):
            ...
            try:
                ...
                # there must be a better way for doing this
                json.dumps(data)
                return data, status
            except Exception as e:
                pass
