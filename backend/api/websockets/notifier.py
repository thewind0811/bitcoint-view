...
class Notifier:
    ...
    def subscribe(self, websocket: WebSocket) -> None:
        log.info(f'Websocket with hash id {hash(websocket)} subscribed to notifier')
    ...
    def unsubscribe(self, websocket: WebSocket) -> None:
        ...
        with suppress(ValueError):
            ...
            log.info(f'Websocket with hash id {hash(websocket)} unsubscribed from notifier')
    ...
    def broadcast(
        ...
        Args:
            ...
            to_send_data:
            success_callback:
            success_callback_args:
            failure_callback:
            failure_callback_args:
...
class WsApp(WebSocketApplication):
    ...
    a different class "Notifier" handling the bulk of the work
    """
    def on_open(self, *args: Any, **kwargs: Any) -> None:
        notifier: Notifier = self.ws.environ['notifier']
        notifier.subscribe(self.ws)
