...
class ModelObserver(BaseObserver):
    ...
    def _connect(self):
        ...
        post_init.connect(self.post_init_receiver, sender=self.model_cls, dispatch_uid=id(self))
    ...
    def database_event(self, instance: Model, action: Action):
        ...
        connection.on_commit(partial(self.post_save_receiver, instance, action))
    ...
    def post_change_receiver(self, instance: Model, action: Action, **kwargs):
        ...
        self.send_messages(instance, old_group_names & new_group_names, Action.UPDATE, **kwargs)

        # the object has been created
        self.send_messages(instance, new_group_names - old_group_names, action.CREATE, **kwargs)

    def send_messages(self, instance: Model, group_names: Set[str], action: Action, **kwargs):
        if not group_names:
            return
    ...
    def serialize(self, instance, action, **kwargs) -> Dict[str, Any]:
        ...
        if self._serializer:
            ...
            message_body = self._serializer(instance).data
        else:
