...
class PatchModelMixin:
    ...
    def patch(self, data: dict, **kwargs) -> Tuple[ReturnDict, int]:
        ...
        instance = self.get_object(data=data, **kwargs)
        serializer = self.get_serializer(
            instance=instance, data=data, partial=True, action_kwargs=kwargs
        )
...
class PaginatedModelListMixin(ListModelMixin):
    ...
    def paginate_queryset(self, queryset, **kwargs) -> Optional[Any]:
...
class StreamedPaginatedModelListMixin(PaginatedModelListMixin):
    @action()
