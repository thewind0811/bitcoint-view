...
def action(atomic: Optional[bool] = None, detached: Optional[bool] = None, **kwargs):
    ...
    def decorator(func):
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            raise ValueError("Only synchronous actions can be detached")
        ...
        async def wrapped_action():
            try:
                ...
                if isinstance(response, tuple):
                    ...
                    await self.reply(
                        data=data, status=status, action=kwargs.get("action"), request_id=kwargs.get("request_id")
                    )
            except asyncio.CancelledError:
                raise
            except Exception as e:
                await self.handle_exception(exc=e, action=kwargs.get("action"), request_id=kwargs.get("request_id"))
