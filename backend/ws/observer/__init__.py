...
from ws.observer.model_observer import ModelObserver
from ws.observer.observer import ObServer
...
def observer(signal: Signal, *kwargs):
    ...
    If the signal you are using supports filtering with `args` and `kwargs` these can be passed
...
def model_observer(model: Type[Model], **kwargs):
    ...
    .. code-block:: python
        # consumers.py
        from ws.consumers import GenericAsyncAPIConsumer
